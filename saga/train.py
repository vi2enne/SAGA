...
import saga.utils as utils
import saga.saga as saga
from saga.models import ANet
...
def evaluate_policy(args,
    ...
    with torch.no_grad():
        ...
        # Initialize lists to store goals and subgoals
        all_subgoals = []
        all_achieved_goals = []
        all_states = []
        ...
        for eval_ep in range(eval_episodes):
            ...
            while not done:
                if step_count % manager_propose_frequency == 0:
                    ...
                    if eval_ep == 0:
                        all_achieved_goals.append(achieved_goal)
                        if manager_policy.absolute_goal:
                            all_subgoals.append(subgoal)
                        else:
                            all_subgoals.append(state[:2] + subgoal)
        ...
        all_achieved_goals = np.array(all_achieved_goals)
        all_goal = np.array(goal)
...
#         env = gym.make(args.env_name, reward_shaping=args.reward_shaping)
    ...
    file_name = "{}_{}_{}_{}".format(args.env_name, args.algo, args.version, args.seed)
    output_data = {"frames": [], "reward": [], "dist": []}

    env.seed(args.seed)
    if hasattr(env, 'sim'):
        env.sim.seed(args.seed)
    ...
    controller_policy = saga.Controller(
        state_dim=state_dim,
            ...
            if total_timesteps != 0 and not just_loaded:
                ...
                if timesteps_since_eval >= args.eval_freq:
                    ...
                    final_x, final_y, final_z, final_subgoal_x, final_subgoal_y, final_subgoal_z = \
                        evaluate_policy(args, env, args.env_name, manager_policy, controller_policy,
    ...
    avg_ep_rew, avg_controller_rew, avg_steps, avg_env_finish, \
    final_x, final_y, final_z, final_subgoal_x, final_subgoal_y, final_subgoal_z = \
        evaluate_policy(args, env, args.env_name, manager_policy, controller_policy, calculate_controller_reward,
